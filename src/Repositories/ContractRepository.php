...
class ContractRepository
{
    ...
    public function addContract(array $params):bool{
        ...
        foreach ($contracts as $contract){
            $tempStart = DateTime::createFromFormat($format, $contract['start'])->setTime(0,0,0,0);
            $tempEnd = DateTime::createFromFormat($format, $contract['end'])->setTime(0,0,0,0);
    ...
    private function updateContractByParams(\PDO $connection, int $id, array $params):bool{
        $columns = '';
    ...
    public function updateContractWithCheck(int $id, array $params):bool{
        ...
        if(!isset($params['start'])){
            $params['start']=DateTime::createFromFormat('Y-m-d',$contracts[$id][0]['start'])->setTime(0,0,0,0);
        }
        if(!isset($params['end'])){
            $params['end']=DateTime::createFromFormat('Y-m-d',$contracts[$id][0]['end'])->setTime(0,0,0,0);
        }
        ...
        foreach ($contracts as $key => $contract){
            if($key==$id){continue;}
            $tempStart = DateTime::createFromFormat('Y-m-d', $contract[0]['start'])->setTime(0,0,0,0);
            $tempEnd = DateTime::createFromFormat('Y-m-d', $contract[0]['end'])->setTime(0,0,0,0);
            if((($params['start']>=$tempStart)&&($params['start']<=$tempEnd))||
                (($params['end']>=$tempStart)&&($params['end']<=$tempEnd))||
                (($params['start']<=$tempStart)&&($params['end']>=$tempEnd))
            ){
        ...
        $params['start']=$params['start']->format('Y-m-d');
        $params['end']=$params['end']->format('Y-m-d');
        $this->updateContractByParams($connection,$id,$params);
        return $connection->commit();
    }
